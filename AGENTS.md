...
- Local dev uses embedded PGlite (`./.pglite`); production uses Postgres via
  `DATABASE_URL`. PGlite allows only ONE process at a time — stop the dev
  server (`pkill -f next-server`) before running scripts or `drizzle-kit push`.
- Schema lives in `src/db/schema.ts`; apply with `npm run db:push`.
